...
pub fn penultimate<T>(list: &List<T>) -> Option<&T> 
{
    match *list {
        Nil => return None,
        Cons(ref head, ref tail) => match **tail {
            Nil => return None,
            Cons(_, ref tail_of_tail) => match **tail_of_tail {
                Nil => return Some(head),
                Cons(_, _) => return penultimate(tail)
            } 
        }
    }
}
...
#[test]
fn penultimate_returns_last_but_one_element2()
{
    assert_eq!(Some(&5), penultimate(&list![1, 1, 2, 3, 4, 5, 8]));
}
